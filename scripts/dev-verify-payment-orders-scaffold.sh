...
if ! schema_json="$(curl -fsS -H 'Accept: application/json' "${API_BASE}/debug/schema-check")"; then
...
category_json="$(curl -fsS -H 'Accept: application/json' "${API_BASE}/event-categories")"
...
payment_json="$(curl -fsS -H 'Accept: application/json' "${API_BASE}/admin/payment-settings")"
...
log_step "[payment-orders-scaffold] creating temporary seating layout + event"
...
layout_json="$(post_json "POST" "/seating-layouts" "$layout_payload")"
...
event_json="$(post_json "POST" "/events" "$event_payload")"
...
public_event_json="$(curl -fsS -H 'Accept: application/json' "${API_BASE}/events/${event_id}")"
