...
Key steps to deploy the frontend and backend:

- Build the frontend: `npm run build` in `frontend/`
- Copy `frontend/build` into the server's public directory (for local dev we use `backend/admin`)
...
| `backend` | Upload to the API directory (e.g., `/public_html/api` or `/home/.../api`). Only overwrite PHP source files, not `uploads/` or `.env`. |

## Do **not** overwrite
- `backend/uploads/` – contains user-uploaded media. Copy new files in but never delete the folder.
- `backend/.env` – keeps environment secrets and DB credentials.
...
3. Upload changed PHP files (`backend/*.php`, `backend/lib/*`, etc.). Skip `uploads/` and `.env`.
4. Clear any PHP opcache if present (via cPanel or `touch backend/index.php`).
5. Run the regression checklist (see `docs/REGRESSION-CHECKLIST.md`).

## Rollback plan
1. Keep a timestamped backup zip of both `frontend/build` and `backend` before deploying (e.g., `midway-deploy-YYYYMMDD-HHMM.zip`).
2. If deployment fails, restore by:
   - Replacing the web root contents with the previous `build` backup.
   - Restoring the previous `backend` folder from the backup zip.
